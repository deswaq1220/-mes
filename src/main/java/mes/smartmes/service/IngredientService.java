...
//            System.out.println("ingredientInput3 = " + ingredientInput);
    ...
    @Transactional
    public void updatePorderStatusAndInsertIngredient1(String porderNo, String planNo) {
...
//            System.out.println("ingredientInput3 = " + ingredientInput);
    ...
    @Scheduled(cron = "*/20 * * * * ?") // 30초 마다 실행
    public void processPordersAutomatically1() {
        LocalDateTime currentDateTime = LocalDateTime.now();
        List<Porder> porders = porderRepository.findByPorderStatusAndPorderDateBefore("입고대기", currentDateTime);
        String planNo = null;
        if (porders != null && !porders.isEmpty()) {
            List<ProductionPlan> plans = productionPlanRepository.findByProdPlanFinYn("작업지시완료(원자재출하)");
            for(ProductionPlan plan : plans){
                planNo = plan.getProdPlanNo();
            }
            for (Porder porder : porders) {
                String porderNo = porder.getPorderNo();
                if(planNo != null){
                    updatePorderStatusAndInsertIngredient1(porderNo, planNo);
                }
